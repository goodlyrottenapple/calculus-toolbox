...
definition dirty :: "nat \<Rightarrow> nat \<Rightarrow> Formula " where
"dirty n j = (if n > j then \<And>\<^sub>F (dirty'_aux n (n-j)) else \<And>\<^sub>F (dirty'_aux n 0))"
...
(*lemma Swapout_R_2aux3:
  fixes loc
  assumes refl_rel: "RelAKA rel \<in> set loc" and "rel alpha a = [alpha]"
  shows "loc \<turnstile>d Y \<turnstile>\<^sub>S forwA\<^sub>S alpha (forwK\<^sub>S a (Structure_ForwA alpha) \<^sup>k X) \<Longrightarrow> loc \<turnstile>d Y \<turnstile>\<^sub>S Structure_ForwA alpha \<^sup>(Suc k) (forwK\<^sub>S a X)"
apply(induct k arbitrary: X Y)
apply(subst k_apply.simps)+
apply simp
proof -
case goal1
show ?case
apply (rule_tac rel=rel in Swapout_R_2aux)
using assms apply (simp, simp)
...
case goal2 
  ...
  case goal2
    ...
    then show ?case unfolding dirty_def using goal2 by simp
  qed
qed
...
(* -------------------- case N = suc, K = 0 -------------------- 
 n children, 1 dirty child *)
  case 0
    ...
    (fboxA\<^sub>F (''father''@nat_to_string (Suc n)) (k_apply (Formula_FboxA (''no''@nat_to_string (Suc n))) 0 (fboxK\<^sub>F (nat_to_string j) (nat_to_string j) \<^sub>F)) )\<^sub>S" by simp
...
(* ------------------ start of claim 1 ------------------ *)
    ...
    from Suc have ih: "\<forall>j\<in>{1..Suc k}.
     loc \<turnstile>d ((dirty (Suc (Suc n)) (Suc k)) \<and>\<^sub>F
     (E (Suc (Suc n)) \<^sup>Suc k vision (Suc (Suc n)))) \<^sub>S \<turnstile>\<^sub>S (fboxA\<^sub>F (''father'' @
    nat_to_string (Suc (Suc n))) Formula_FboxA (''no'' @ nat_to_string (Suc (Suc n))) \<^sup>k fboxK\<^sub>F nat_to_string j nat_to_string j \<^sub>F )\<^sub>S" sorry
...
(****************************************************************** NEW ******************************************************************)
    ...
    (*have LEM363: *)have lem363a: "loc \<turnstile>d ( ((nat_to_string (Suc (Suc k)) \<^sub>F) \<rightarrow>\<^sub>F \<bottom>\<^sub>F ) \<rightarrow>\<^sub>F ( (fboxA\<^sub>F (''father'' @
     nat_to_string (Suc (Suc n))) ( Formula_FboxA (''no'' @ nat_to_string (Suc (Suc n))) \<^sup>k fboxK\<^sub>F nat_to_string 1 (nat_to_string 1 \<^sub>F)) ))) \<and>\<^sub>F 
     (fdiamA\<^sub>F (''father'' @
     nat_to_string (Suc (Suc n))) ( Formula_FdiamA (''no'' @ nat_to_string (Suc (Suc n))) \<^sup>k (no (Suc(Suc n)) )) )\<^sub>S
...
sorry
    ...
    apply (rule_tac derivable.FdiamA_L)
    apply (rule_tac derivable.Balance)
    apply (rule_tac derivable.FdiamA_L)
    apply (rule_tac derivable.Balance)
    apply (rule_tac derivable.Id) *)
...
(* ------------------ start of claim 3 ------------------ *)
 ...
 ( (fboxA\<^sub>F (''father'' @
     nat_to_string (Suc (Suc n)))  fboxK\<^sub>F (nat_to_string (Suc (Suc k))) (  Formula_FboxA (''no'' @ nat_to_string (Suc (Suc n))) \<^sup>Suc k (nat_to_string (Suc (Suc k)) \<^sub>F)) ) )\<^sub>S
    "
apply (rule_tac derivable.FboxA_R)
apply (rule_tac derivable.Back_forw_A2)
apply (rule_tac derivable.FboxK_R)
...
have claim4: "loc \<turnstile>d ( (fboxA\<^sub>F (''father'' @
     nat_to_string (Suc (Suc n)))  fboxK\<^sub>F (nat_to_string (Suc (Suc k))) (  Formula_FboxA (''no'' @ nat_to_string (Suc (Suc n))) \<^sup>Suc k (nat_to_string (Suc (Suc k)) \<^sub>F)) ) )\<^sub>S \<turnstile>\<^sub>S 
